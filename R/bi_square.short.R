...
mixlinrb_bi_old<-function(formula,data,nc=2, numini=20){ 
	nx=ncol(data)-1; n = nrow(data) 
	p=nx+1; n1=2*p;
	...
	est= sol[bestindex[1],];
	out=list(theta=matrix(est,nrow=nc),estall=theta, uniqueest=sol, countuniqueest=num,uniqueestindex=solindex, bestindex= solindex[bestindex],estindex=ind);
	return(out)
}
